...
- After a deploy, apply pending one-time migrations: `wrangler d1 execute studioos-db --file=cloudflare-worker/sql/<file>.sql --remote --env=""`. As of 2026-05-06 all known migrations through T3/T11/T12/T13-15 + perf_indexes + backfill_activity_user_ids + lp_investors_seal are applied to remote D1. Pending: _none_. (Note: re-running `t13_t14_t15.sql` will error "duplicate column name: mentor_id" on the ALTER — D1 rolls the whole file back, but every CREATE in it is `IF NOT EXISTS` so the file is effectively idempotent except for that ALTER.)
