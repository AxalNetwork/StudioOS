...
- 2026-05-06 Task #5 (deferred majors): Between Task #4 wrapping and Task #5 starting, four major-bump PRs were already merged into main and verified clean: **react 19.0→19.2.6 + react-router 7.1→7.15 (#6)**, **recharts 2→3.8.1 (#16)**, **jose 5→6.2.3 (#11)**, **typescript 5→6.0.3 (#13)**. Worker `tsc --noEmit` and frontend `vite build` both pass; both audits report 0 vulnerabilities. Two majors **remain intentionally deferred** under the GitHub `deferred-upgrade` label with tracking comments: PR #15 lucide-react 0→1 (icon renames risk; no security alert) and PR #12 build group / vite 6→7 (biggest build-tool blast radius; closes vite alerts #4+#5 when picked up). Do not auto-merge or close those two — Dependabot will keep them rebased until the planned upgrade window.
