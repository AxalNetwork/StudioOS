...
- **HubSpot dual-auth** (Task #17) — HubSpot is a `supports_pat: true` provider in `registry.ts`. `providers/hubspot.ts::connect()` branches on `input.api_key` FIRST (Private App path: long-lived bearer, `is_private_app: true`, no refresh, validated via `/oauth/v1/access-tokens/{token}`); only falls through to `input.oauth_code` (Marketplace OAuth path). `getActiveAccessToken()` short-circuits the refresh+lease code path when `creds.is_private_app === true` OR when there is no `refresh_token`/`expires_at` — never attempt to refresh a PAT row. Frontend modal renders the PAT field with HubSpot-specific copy ("Settings → Integrations → Private Apps") whenever `provider.supports_pat`. Marketplace OAuth is currently blocked by HubSpot's unpublished-app rule on non-test portals; PAT is the only working path until the public app is published.
