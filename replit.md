...
- **Apex routing** — Worker serves the SPA on both `app.axal.vc/*` (custom domain) AND `axal.vc/{api,app,dashboard,admin,register,login}{,/*}` (path-scoped routes on the proxied apex CNAME → `axalnetwork.github.io`). Jekyll keeps `axal.vc/` and any other unrouted paths. Adding a new top-level app route means adding TWO patterns to `[[env.production.routes]]` in `wrangler.toml` (exact + `/*`) so Jekyll pages with similar prefixes (`/registered-*`) aren't hijacked. `/api/*` MUST stay routed on the apex or SPA fetches from `axal.vc/dashboard` 404 on Jekyll. **Canonical-host status (Phase 2 in progress)**: code defaults emit `axal.vc` URLs, but prod env vars in `wrangler.toml` still pin `APP_URL`/`PUBLIC_BASE_URL` to `app.axal.vc` to avoid breaking ~10 integration providers whose OAuth `redirect_uri` registrations still point there. `routes/auth_google.ts` splits `appUrl()` (post-callback redirect → axal.vc) from `oauthCallbackHost()` (redirect_uri → app.axal.vc, overridable via `OAUTH_CALLBACK_BASE_URL`); other providers still derive `redirect_uri` from `APP_URL`. Edge 301 middleware in `index.ts` redirects `app.axal.vc/*` → `axal.vc/*` for non-`/api/*` paths, but only fires for non-asset paths due to `run_worker_first = ["/api/*", "/landing/*"]` — SPA-bookmark convergence needs a Cloudflare bulk-redirect rule on the `app.axal.vc` zone. Cutover sequence: update provider registrations → flip env vars in wrangler.toml → add bulk-redirect → deploy.
