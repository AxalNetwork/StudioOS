...
- **Founder KYC stage-gate** (`FounderPortal.jsx`): new `FounderKycBanner` only renders when the logged-in user is `role==='founder'`, `kyc_status !== 'approved'`, `access_level !== 'limited'`, AND `api.listProjects()` returns at least one project with `stage` in `('BUILD','LAUNCH')`. Admin "View as Founder" sessions never see it (the stored localStorage user is the admin). Sign-time KYC gates remain server-authoritative on the worker / FastAPI; this banner is only the in-app nudge.
