...
- **Pending / partial migrations**: `039_project_cascade.sql` partial (deleted_at + index applied; FK-cascade rebuilds blocked by D1's BEGIN/COMMIT rejection); `034_unmounted_routes.sql` unapplied (missing `owner_user_id`); `056_customer_chat_threads.sql` unapplied; `061_google_sub.sql` **superseded by 065** (don't re-apply); `062`/`063`/`064` unapplied.
