...
| F3 | Signed contracts in R2 had no minted-URL primitive. Worker-streamed download was implemented per-route (esign), but no general "give the admin a 5-minute one-shot link" path existed. | **MEDIUM** | New `services/signedDownload.ts` + `routes/files.ts` mount signed download URLs at `/api/files/dl/:token`. TTL hard-clamped to 5 minutes. Single-use is **best-effort** (KV get→delete is not CAS — see §3.1). Every download writes to `activity_logs` so duplicate consumption is detectable post-hoc. **OPEN**: back with a Durable Object if strictly-once semantics are ever required. |
