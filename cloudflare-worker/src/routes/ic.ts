...
r.post('/', async (c) => {
  try {
    ...
    // Optional: seed the memo from the latest stored scoring deal-memo.
    if (!memo && (body.from_scoring || c.req.query('from_scoring')) && projectId != null) {
      const snap = await c.env.DB.prepare(
        "SELECT deal_memo FROM scoring_snapshots WHERE project_id = ? AND deal_memo IS NOT NULL ORDER BY created_at DESC LIMIT 1"
      ).bind(projectId).first<{ deal_memo: string }>().catch(() => null);
      if (snap?.deal_memo) memo = String(snap.deal_memo).slice(0, 20000);
    }
