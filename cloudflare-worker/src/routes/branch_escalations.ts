...
import { BRANCH_ONLY, branchOf } from '../util/branch';
...
/** Every route here is branch-only: HQ raises nothing to itself. */
function requireBranchTier(c: { env: Env }): string {
  const code = branchOf(c.env);
  // The SHARED constant, so `AUTH_ERROR_STATUSES` maps it to 403. Its first
  // version threw its own sentence and `mapError` answered 400 — the exact
  // defect D110 found across 31 route files, caught here by the test.
...
r.post('/escalations', async (c) => {
  try {
    ...
    if (!hq || typeof hq.escalate !== 'function') {
      ...
      try {
        const res = await hq.escalate(branchOf(c.env)!, {
