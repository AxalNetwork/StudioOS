/**
   * Task #14 (AA-1) — LinkedIn job postings connector.
   *
   * Cadence: daily. Dimensions: talent. Weight: 0.6.
 * Paid contract — fetchLive throws until billing engaged.
  ...
  function buildRows(sectors: string[], now: Date): CommonRow[] {
    ...
    return sectors.map((sector) =>
      row({
        ...
        ts: now.toISOString(),
        citation_url: 'https://axal.vc/market-intel/sources/linkedin_jobs',
      })
    );
  }
  ...
  daily_cap: 500,
    fetchLive: async () => {
      ...
      throw new Error('linkedin_jobs_paid_contract_not_signed');
    },
