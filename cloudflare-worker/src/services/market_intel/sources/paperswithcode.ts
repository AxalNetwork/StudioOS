/**
   * Task #14 (AA-1) — Papers with Code benchmarks connector.
   *
   * Cadence: weekly. Dimensions: research. Weight: 0.5.
   *
   ...
   * dev. Set MI_FLAG_PAPERSWITHCODE=live in production env to engage fetchLive.
   */
  import type { Env } from '../../../types';
  import { registerSource, type CommonRow } from '../registry';
  import { row, seededAround, doy } from './_helpers';

  const KEY = 'paperswithcode';
  ...
  function buildRows(sectors: string[], now: Date): CommonRow[] {
    ...
    return sectors.map((sector) =>
      row({
        ...
        ts: now.toISOString(),
        citation_url: 'https://axal.vc/market-intel/sources/paperswithcode',
      })
    );
  }
  ...
  registerSource({
    key: KEY,
    display_name: "Papers with Code benchmarks",
    category: 'research',
    cadence: 'weekly',
    dimensions: ['research'],
    weight: 0.5,
    ...
    fetchLive: async (_env, { sectors }) => {
      // TODO: wire real Papers with Code benchmarks client. Falls back to stub semantics
