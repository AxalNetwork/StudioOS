/**
   * Task #14 (AA-1) — SimilarWeb traffic connector.
   *
   * Cadence: weekly. Dimensions: demand. Weight: 0.55.
 * Paid contract — fetchLive throws until billing engaged.
  ...
  function buildRows(sectors: string[], now: Date): CommonRow[] {
    ...
    return sectors.map((sector) =>
      row({
        ...
        ts: now.toISOString(),
        citation_url: 'https://axal.vc/market-intel/sources/similarweb',
      })
    );
  }
  ...
  registerSource({
    key: KEY,
    display_name: "SimilarWeb traffic",
  ...
  daily_cap: 500,
    fetchLive: async () => {
      ...
      throw new Error('similarweb_paid_contract_not_signed');
    },
