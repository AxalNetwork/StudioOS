/**
   * Task #14 (AA-1) — Shopify Partners app installs connector.
   *
   * Cadence: weekly. Dimensions: supply, demand. Weight: 0.45.
   *
   ...
   * dev. Set MI_FLAG_SHOPIFY_PARTNERS=live in production env to engage fetchLive.
   */
  import { registerSource, type CommonRow } from '../registry';
  import { row, seededAround, doy } from './_helpers';

  const KEY = 'shopify_partners';
  ...
  function buildRows(sectors: string[], now: Date): CommonRow[] {
    ...
    return sectors.map((sector) =>
      row({
        ...
        ts: now.toISOString(),
        citation_url: 'https://axal.vc/market-intel/sources/shopify_partners',
      })
    );
  }
  ...
  registerSource({
    key: KEY,
    display_name: "Shopify Partners app installs",
    category: 'commerce',
    cadence: 'weekly',
