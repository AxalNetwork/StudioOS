...
-- Venture Risk Rating (migration 114) — 10-layer risk aggregation. Latest
-- assessment row per project is "current"; overrides are sticky per layer.
CREATE TABLE IF NOT EXISTS venture_risk_assessments (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    uid TEXT UNIQUE NOT NULL DEFAULT (lower(hex(randomblob(16)))),
    project_id INTEGER NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    overall_risk REAL NOT NULL DEFAULT 0,
    overall_band TEXT NOT NULL DEFAULT 'medium',
    derisk_score REAL NOT NULL DEFAULT 0,
    derisk_pct REAL NOT NULL DEFAULT 0,
