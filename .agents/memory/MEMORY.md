...
- [Test & tooling quirks](test-tooling-quirks.md) — workspace is now nodejs-22 (upgraded); wrangler requires Node 22+; cloudflare-worker install needs --legacy-peer-deps; node:sqlite binds ?N by occurrence not number; Vite 8 silently ignores esbuild.target (use oxc.target — Safari blank page).
