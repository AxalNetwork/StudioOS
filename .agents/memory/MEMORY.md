...
- [Investor canvas empty states](investor-canvas-empty-states.md) — preserve each canvas workspace’s full composition when live records are absent; use honest in-place empty states, never legacy wrappers.
- [Workspace handoffs](workspace-handoffs.md) — carry loaded scope into canvas-to-editor navigation; immediate refetches can turn rate limits into false no-record states.
