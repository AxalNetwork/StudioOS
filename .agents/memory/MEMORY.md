...
- [Spin-Out Lab conventions](spinout-lab-conventions.md) — /spinout-lab is a bare route by design (no shell sidebar); its Worker test slices exact source strings, so new /state fields go in the wire handler.
