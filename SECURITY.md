...
Cloudflare Worker at `axal.vc/api/*` and the frontend at `app.axal.vc`
are continuously updated; there is no notion of a "supported version"
for end users. Researchers should target the live production surface or
...
- Personal Advisor chatbot — prompt injection, scope-lock bypass, tool
  misuse, exfiltration of other users' data.
