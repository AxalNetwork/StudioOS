...
| `zoneFilterBuilder.js` | The filter half of D49. A filter that narrows real rows is a chip; one with no store is prose naming the missing record, because a dead filter returns an empty set and an empty set reads as an answer. |
