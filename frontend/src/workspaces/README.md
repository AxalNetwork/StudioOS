...
| `founderZoneFilters.js` | The founder profile's eighteen zones, 75 canvas labels. Sample figures (`All 14`, `Aug 2026`) and sample names (`Backend`, `Agencies`) are never copied — the page supplies the real count or the stored names. |
| `investorZoneFilters.js` | The investor profile's Fund, Portfolio and Deals-pipeline zones. D52's rule lives here: a label is not dead until the column, the write path and the client method have each been looked for — three of `/deals/pipeline`'s five read fields the page had already loaded. Its eleven remaining canvas routes are held in the guard's `excluded` set with a reason each. |
