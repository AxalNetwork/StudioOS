...
export default function FounderRiskBadge({ dealId }) {
  ...
  if (!profile) {
    return (
      <button
        onClick={pull}
        disabled={pulling}
        className="text-xs px-2 py-0.5 rounded border border-gray-300 text-gray-600 hover:bg-gray-50 flex items-center gap-1 disabled:opacity-50"
  ...
  return (
    <div className="relative inline-block" ref={popRef}>
      ...
      {open && (
        <div className="absolute z-30 right-0 mt-1 w-80 bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-left">
          ...
          {profile.domain_tags?.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2">
              {profile.domain_tags.map(t => (
                <span key={t} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-700">
                  #{t}
                </span>
              ))}
            </div>
          ...
          {profile.notable_signals?.length > 0 && (
            <ul className="text-[11px] text-gray-700 list-disc pl-4 mb-2 space-y-0.5">
