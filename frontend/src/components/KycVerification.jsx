...
// The list moved to lib/countries.js so the investor onboarding wizard —
// which asks the same question — can reach it. Aliased rather than
// renamed throughout: ALL_COUNTRIES is used in the filter below and in
// the picker, and a rename would be churn with no reader benefit.
...
/**
 * Identity Verification (KYC / AML) form + status.
 *
 * Task #25 — extracted out of the standalone KYCPage so it can render both on
 * the `/kyc` route (full page chrome) AND embedded inside the Trust Center
 * "Identity" tab. Pass `embedded` to drop the page header + outer max-width
 * wrapper (the Trust Center Section already supplies a heading), keeping a
 * single source of truth for the form, status card, and the investor-only gate.
 */
export default function KycVerification({ embedded = false }) {
  ...
  if (role && role !== 'investor' && role !== 'admin') {
    ...
    if (embedded) return notRequired;
    return (
