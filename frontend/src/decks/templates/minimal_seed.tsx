// frontend/src/decks/templates/minimal_seed.tsx
//
// Minimal Seed — exactly 6 slides, one investor question each, designed
...
//
// Standalone `MinimalSeedDeckApp` (Framer Motion shell with prev/next
// nav + AnimatePresence transitions) is also exported for any consumer
// that wants the live single-screen viewer outside the print pipeline.
...
// Per-slide prop shape (renamed from the paste's `DeckProps` so it
// doesn't collide with the registry-wide `DeckProps` imported from
// `../DeckBase` and re-used by `Deck_minimal_seed` below).
type SlideProps = {
...
const fmtNum = (n?: number) => (n == null || isNaN(n as number) ? '—' : n.toLocaleString());
const fmtPct = (n?: number) => (n == null || isNaN(n as number) ? '—' : `${n}%`);
...
//
// Carries `data-slide-frame=""` + fixed 1920×1080 size + `pageBreakAfter:
// always` so the existing print pipeline in `PitchDeckPrintPage.jsx`
// (keyboard nav, page-break, PDF export) treats each Slide as a frame
// the same way it treats `Slide16x9` from `DeckBase.tsx`.
// ─────────────────────────────────────────────────────────────────
...
const TimelineDots: React.FC<{ items: { date?: string; year?: string; label?: string; event?: string }[]; activeIdx?: number }> = ({
  items,
  activeIdx = -1,
}) => (
  <div className="relative w-full">
    ...
    >
      {items.map((m, i) => {
        const head = (m.date || m.year || '') as string;
        const body = (m.label || m.event || '') as string;
        return (
...
const Slide1Company: React.FC<SlideProps> = ({ data = {}, editable, onEdit }) => (
...
const Slide2Problem: React.FC<SlideProps> = ({ data = {}, editable, onEdit }) => (
...
const Slide3Solution: React.FC<SlideProps> = ({ data = {}, editable, onEdit }) => {
...
const Slide4Traction: React.FC<SlideProps> = ({ data = {}, editable, onEdit }) => {
  ...
  return (
    <SlideFrame index={3} total={6} label="Traction" question="What evidence exists?" company={data.company}>
      ...
      <div className="mt-6 grid grid-cols-2 gap-6">
        <div className="p-4 rounded-xl" style={{ background: SURFACE, border: `1px solid ${HAIRLINE}` }}>
          <div style={{ fontSize: 10, color: SUBTLE, letterSpacing: 2, fontFamily: FONT_MONO }}>
            REVENUE ($K)
          </div>
          <AreaChart data={rev} color={ACCENT} />
        </div>
        <div className="p-4 rounded-xl" style={{ background: SURFACE, border: `1px solid ${HAIRLINE}` }}>
          <div style={{ fontSize: 10, color: SUBTLE, letterSpacing: 2, fontFamily: FONT_MONO }}>
            ACTIVE USERS
          </div>
          <AreaChart data={users} color="#7C8CE0" />
        </div>
      </div>
...
const Slide5Team: React.FC<SlideProps> = ({ data = {}, editable, onEdit }) => {
...
const Slide6Ask: React.FC<SlideProps> = ({ data = {}, editable, onEdit }) => {
...
// ─────────────────────────────────────────────────────────────────
// SAMPLE_DATA — rich defaults so the deck renders convincingly when
// rendered standalone, and so unmapped fields in the registry adapter
// (`Deck_minimal_seed` below) get a sensible per-field fallback.
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Standalone deck shell — keyboard nav, dot pagination, motion
// transitions. Kept exported for any consumer that wants the live
// single-screen viewer outside the print pipeline.
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
// Registry adapter — `Deck_minimal_seed`
//
...
//
// `ask_amount` is also accepted (legacy editor field name) and maps to
// `ask_amount_usd` to match the rest of the deck registry.
// ─────────────────────────────────────────────────────────────────
...
function mergeWithSample(input: Record<string, any> | undefined): MinimalSeedData {
  const out: any = { ...SAMPLE_DATA };
  if (!input) return out;
  // Legacy field-name normalisation — runs through the same merge
  // path below so shape rules stay consistent.
  const src: any = { ...input };
  if (src.ask_amount != null && src.ask_amount_usd == null) {
    src.ask_amount_usd = src.ask_amount;
  }
  for (const k of Object.keys(src)) {
    ...
    if (NESTED_OBJECT_FIELDS.has(k as keyof MinimalSeedData) && isPlainObject(v)) {
      // Deep-merge per subfield over the sample object so partial
      // payloads keep the unfilled subfields populated.
      const sampleObj = isPlainObject(out[k]) ? out[k] : {};
      const merged: Record<string, any> = { ...sampleObj };
...
export const Deck_minimal_seed: React.FC<RegistryDeckProps> = ({ data, editable, onEdit }) => {
  ...
  return (
    <>
      <Slide1Company data={merged} editable={editable} onEdit={onEdit} />
      <Slide2Problem data={merged} editable={editable} onEdit={onEdit} />
      <Slide3Solution data={merged} editable={editable} onEdit={onEdit} />
      <Slide4Traction data={merged} editable={editable} onEdit={onEdit} />
      <Slide5Team data={merged} editable={editable} onEdit={onEdit} />
      <Slide6Ask data={merged} editable={editable} onEdit={onEdit} />
    </>
  );
};

export default Deck_minimal_seed;
