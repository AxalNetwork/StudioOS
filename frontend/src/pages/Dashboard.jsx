...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                {proprietary_deal_flow.slice(0, 6).map(d => (
                  <Link key={d.id} to={`/projects/${d.id}`} className="flex items-center gap-3 py-3 hover:bg-gray-50 -mx-2 px-2 rounded transition-colors">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{d.sector || 'Unclassified'} • {d.stage || 'stage'}</div>
                    </div>
          ...
          <Card title="Performance Analytics" icon={BarChart3}>
            ...
            <div className="mb-4">
              <div className="text-xs text-gray-500 dark:text-gray-400 mb-2 font-medium">Earnings — Last 30 Days</div>
              <Sparkline series={performance_analytics.earnings_series_30d} />
            </div>
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <div className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Recent Commissions</div>
                {performance_analytics.recent_commissions.length === 0 ? <Empty text="None yet." /> : (
                  <div className="space-y-1">
                    {performance_analytics.recent_commissions.slice(0, 5).map(c => (
                      <div key={c.id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                        <div className="truncate flex-1 text-gray-700 dark:text-gray-300">{c.description}</div>
              ...
              <div>
                <div className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Top Referrers in Your Chain</div>
                {performance_analytics.top_referrers.length === 0 ? <Empty text="None yet." /> : (
                  <div className="space-y-1">
                    {performance_analytics.top_referrers.map(r => (
                      <div key={r.id} className="flex items-center justify-between text-xs py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                        <div className="truncate flex-1 text-gray-700 dark:text-gray-300">{r.name || r.email}</div>
                        <div className="text-gray-500 dark:text-gray-400">{r.downstream} downstream</div>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="space-y-6">
              ...
              {ai_scored_opportunities.length === 0 ? <Empty text="Set your investor preferences to see AI-scored matches." /> : (
                <div className="space-y-3">
                  {ai_scored_opportunities.slice(0, 5).map(m => (
                    <div key={m.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 hover:border-violet-400 transition-colors">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <div className="font-medium text-sm text-gray-900 dark:text-gray-100 truncate">{m.deal_name || `Deal #${m.deal_id}`}</div>
                        <ScorePill score={m.score} />
                      </div>
                      {m.explanation && <div className="text-xs text-gray-600 dark:text-gray-400 line-clamp-2">{m.explanation}</div>}
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-[10px] uppercase text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">{m.score_type}</span>
          ...
          <Card title="Syndication Tools" icon={Users} link="/network-effects" linkLabel="Network Effects">
              ...
              <div className="mb-4">
                ...
                {syndication_tools.ai_recommendations.slice(0, 3).map(s => (
                  <div key={s.id} className="border border-violet-200 bg-violet-50/50 rounded-lg p-2 mb-2">
                    <div className="flex items-start justify-between gap-2"><div className="font-medium text-xs text-gray-900 dark:text-gray-100">{s.name}</div>{s.score != null && <ScorePill score={s.score} small />}</div>
                    <div className="text-[10px] text-gray-600 dark:text-gray-400 mt-1 line-clamp-1">{s.description}</div>
                  </div>
                ))}
              </div>
            )}

            <div className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Open Syndicates ({syndication_tools.open_syndicates.length})</div>
            {syndication_tools.open_syndicates.length === 0 ? <Empty text="None open right now." /> : (
              <div className="space-y-2">
                {syndication_tools.open_syndicates.slice(0, 4).map(s => {
                  ...
                  return (
                    <Link key={s.id} to="/network-effects" className="block border border-gray-200 dark:border-gray-700 rounded-lg p-2 hover:border-violet-400 transition-colors">
                      <div className="font-medium text-xs text-gray-900 dark:text-gray-100 truncate">{s.name}</div>
                      <div className="text-[10px] text-gray-500 dark:text-gray-400">{s.members} members • ${(s.committed_cents/100).toLocaleString()}{s.target_cents && ` / $${(s.target_cents/100).toLocaleString()}`}</div>
                      {s.target_cents && <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-1 mt-1 overflow-hidden"><div className="h-full bg-violet-500" style={{width: `${pct}%`}} /></div>}
                    </Link>
                  );
                })}
              </div>
            ...
            {syndication_tools.my_memberships?.length > 0 && (
              <div className="mt-4">
                <div className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">My Commitments</div>
                {syndication_tools.my_memberships.slice(0, 3).map(m => (
                  <div key={m.id} className="text-xs flex justify-between py-1 border-b border-gray-100 dark:border-gray-800 last:border-0">
                    <span className="truncate">{m.name}</span>
                    <span className="font-bold text-gray-900 dark:text-gray-100 ml-2">${(m.commitment_cents/100).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Task #5 — pinned personal assistant. Persists across reloads via
          localStorage (open/closed + last conversation uid) and is hidden
...
function Sparkline({ series }) {
  ...
  return (
    <div>
      <div className="flex items-end gap-px h-16 bg-gray-50 dark:bg-gray-800 rounded p-1">
        {data.map((d, i) => (
      ...
      <div className="flex justify-between text-[10px] text-gray-500 dark:text-gray-400 mt-1"><span>30 days ago</span><span>Total: ${(total/100).toFixed(2)}</span><span>Today</span></div>
    </div>
  );
}
...
function Empty({ text }) { return <div className="text-xs text-gray-500 dark:text-gray-400 py-4 text-center">{text}</div>; }
