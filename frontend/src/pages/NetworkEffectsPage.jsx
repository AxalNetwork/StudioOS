    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">Compounding referrals, syndicate formation, and operator/advisor marketplace.</p>
        </div>
      </div>
...
function Compounding() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-5 dark:bg-gray-900 dark:border-gray-800">
        ...
        {data.bonuses.length === 0 ? (
          <div className="text-sm text-gray-500 dark:text-gray-400 py-6 text-center">No compounding bonuses yet. They'll fire when L2/L3 referrals convert.</div>
        ) : (
            ...
            {data.bonuses.map(b => (
              <div key={b.id} className="py-2 flex items-center justify-between text-sm">
                <div>
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">{new Date(b.created_at).toLocaleString()}</div>
                </div>
      ...
      <div className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-5 dark:bg-gray-900 dark:border-gray-800">
        ...
        {total === 0 ? (
          <div className="text-sm text-gray-500 dark:text-gray-400 py-6 text-center">Share your referral link to start building your chain.</div>
        ) : (
          [1, 2, 3].map(lvl => data.levels[lvl]?.length > 0 && (
            <div key={lvl} className="mb-4 last:mb-0">
              <div className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400 font-semibold mb-2">Level {lvl} ({data.levels[lvl].length})</div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {data.levels[lvl].map(p => (
                  <div key={p.id} className="bg-gray-50 dark:bg-gray-900 rounded-lg p-2 text-xs">
                    <div className="font-medium text-gray-900 truncate dark:text-gray-100">{p.name || p.email}</div>
                    <div className="text-gray-500 dark:text-gray-400 truncate">{p.email}</div>
...
function Syndicates() {
  ...
  return (
    <div>
      ...
      {list.length === 0 ? (
        <div className="text-sm text-gray-500 dark:text-gray-400 py-12 text-center bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl dark:border-gray-800">No syndicates yet. Create one to pool capital with other investors.</div>
      ) : (
...
function SyndicateCard({ s, onOpen }) {
  ...
  return (
    <div onClick={onOpen} className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-5 cursor-pointer hover:shadow-md transition-shadow dark:bg-gray-900 dark:border-gray-800">
      <div className="flex items-start justify-between mb-2">
        <h3 className="font-semibold text-gray-900 dark:text-gray-100">{s.name}</h3>
        <span className={`text-xs px-2 py-0.5 rounded ${s.status === 'open' ? 'bg-emerald-100 text-emerald-700' : s.status === 'funded' ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>{s.status}</span>
      </div>
      {s.description && <p className="text-xs text-gray-600 dark:text-gray-400 line-clamp-2 mb-3">{s.description}</p>}
    ...
    <Drawer title={s?.name || 'Loading…'} onClose={onClose}>
      {loading || !s ? <Loading text="" /> : (
        <div className="space-y-5">
          ...
          {s.status === 'open' && (
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 dark:border-gray-800">
          ...
          <div>
            ...
            {s.members.length === 0 ? <div className="text-xs text-gray-500 dark:text-gray-400">No members yet.</div> : (
              <div className="divide-y divide-gray-100">
                {s.members.map(m => (
                  <div key={m.id} className="py-2 flex items-center justify-between text-sm">
                    <div><div className="font-medium text-gray-900 dark:text-gray-100">{m.name}</div><div className="text-xs text-gray-500 dark:text-gray-400">{m.email}</div></div>
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-4 dark:border-gray-800">
            ...
            {recs && !recs.error && (
              <div className="mt-3 space-y-2">
                ...
                {recs.candidates.map(c => (
                  <div key={c.id} className="bg-gray-50 dark:bg-gray-900 rounded p-2 text-xs flex items-center justify-between">
                    <div><div className="font-medium">{c.name}</div><div className="text-gray-500 dark:text-gray-400">{c.email} • {c.role}</div></div>
                    {c.score && <ScorePill score={c.score} />}
                  </div>
                ))}
                {recs.candidates.length === 0 && <div className="text-xs text-gray-500 dark:text-gray-400">No candidates available.</div>}
              </div>
            )}
...
function MarketplaceSearch() {
  ...
  return (
    <div>
      <div className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-4 mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end dark:bg-gray-900 dark:border-gray-800">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Skill</label>
          <input
            placeholder="e.g. React, GTM, Finance"
            value={filters.skill}
            onChange={e => setFilters(f => ({...f, skill: e.target.value}))}
            className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-violet-500 focus:ring-2 focus:ring-violet-100 focus:outline-none transition dark:border-gray-800"
          />
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg pl-3 pr-9 py-2 text-sm text-gray-800 focus:border-violet-500 focus:ring-2 focus:ring-violet-100 focus:outline-none transition cursor-pointer dark:border-gray-800 dark:text-gray-200"
            >
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Availability</label>
          <div className="relative">
            <select
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Min rating</label>
          <div className="flex gap-2 items-center">
            <input
              ...
              className="flex-1 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-violet-500 focus:ring-2 focus:ring-violet-100 focus:outline-none transition dark:border-gray-800"
            />
            <button
              onClick={search}
      ...
      {loading ? <Loading text="Searching…" /> : results.length === 0 ? (
        <div className="text-sm text-gray-500 dark:text-gray-400 py-12 text-center bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl dark:border-gray-800">No operators match. Try clearing filters.</div>
      ) : (
...
function ProfileCard({ p, onIntro }) {
  return (
    <div className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-4 dark:bg-gray-900 dark:border-gray-800">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{p.title || p.role}</div>
        </div>
      ...
      {p.bio && <p className="text-xs text-gray-600 dark:text-gray-400 line-clamp-2 mb-2">{p.bio}</p>}
      <div className="flex flex-wrap gap-1 mb-3">
        {(p.skills || []).slice(0, 5).map(s => <span key={s} className="text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-700 px-1.5 py-0.5 rounded dark:text-gray-300">{s}</span>)}
      </div>
...
function MyMarketplaceProfile() {
  ...
  return (
    <div className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-5 max-w-2xl dark:bg-gray-900 dark:border-gray-800">
  ...
  return (
    <div className="max-w-2xl">
      <div className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-5 dark:bg-gray-900 dark:border-gray-800">
        ...
        <p className="text-xs text-gray-600 dark:text-gray-400 mb-4">Describe what you need; AI picks the top 3 from the marketplace.</p>
      ...
      {result && (
        <div className="mt-4 space-y-3">
          {result.recommendations.map(r => (
            <div key={r.user_id} className="bg-white border border-gray-200 dark:border-gray-700 rounded-xl p-4 dark:bg-gray-900 dark:border-gray-800">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div><div className="font-semibold">{r.profile.name}</div><div className="text-xs text-gray-500 dark:text-gray-400">{r.profile.title}</div></div>
              </div>
...
function Pill({ v }) {
  const colors = { approved: 'bg-emerald-100 text-emerald-700', converted: 'bg-violet-100 text-violet-700', pending: 'bg-amber-100 text-amber-700', rejected: 'bg-red-100 text-red-700', not_started: 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400' };
  return <span className={`text-[10px] px-2 py-0.5 rounded ${colors[v] || 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>{(v || '').replace('_', ' ')}</span>;
}
function ScorePill({ score }) {
  const c = score >= 80 ? 'bg-emerald-100 text-emerald-700' : score >= 60 ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400';
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto dark:bg-gray-900" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between dark:border-gray-800"><h2 className="text-lg font-semibold">{title}</h2><button onClick={onClose} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:text-gray-100"><X size={18} /></button></div>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/40 flex justify-end" onClick={onClose}>
      <div className="bg-white w-full max-w-2xl h-full overflow-y-auto dark:bg-gray-900" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white z-10 dark:border-gray-800 dark:bg-gray-900"><h2 className="text-lg font-semibold">{title}</h2><button onClick={onClose} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:text-gray-100"><X size={18} /></button></div>
        <div className="p-6">{children}</div>
      </div>
    </div>
  );
