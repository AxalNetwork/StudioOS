    ...
    <div className="fr-capital-shell">
      <section className="fr-capital-main">
        <header className="fr-capital-header">
          ...
          <div className="fr-capital-title-row"><div><h1>Data room</h1><p className="fr-capital-subtitle">Permission matrix, per-investor grants, view analytics and gap analysis.</p></div></div>
          ...
          <ZoneToolbar
              filters={founderZoneFilters('raise/data-room', { value: view, onChange: setView })}
              actions={founderZoneActions('raise/data-room', { query, view: { scope: project?.name, header: ['Investor', 'Email', 'Status', 'NDA signed', 'Expires'], rows: grants, cells: (g) => [g.investor_name, g.investor_email, g.status, g.nda_signed ? 'yes' : 'no', g.expires_at] } })}
            />
        </header>
...
function RoomContent({ project, files, folders, grants, activeGrants, access, fileActivity, mostViewed, roomError, view, setView, query }) {
  return <div className="fr-capital-content">
    <div className="fr-capital-context"><div><span className="fr-capital-label">Selected startup</span><strong data-testid="text-data-room-project">{text(project.name)}</strong><span>{text(project.sector, 'Sector not recorded')}</span></div><div className="fr-capital-context-right"><span className="fr-capital-label">Room source</span><strong>{roomError ? 'Data-room source unavailable' : text(project.uid, 'Identifier unavailable')}</strong><span>{roomError ? 'No room records inferred' : 'Project-scoped Worker room'}</span></div></div>
...
function PermissionTable({ files, grants, artifactOnly, roomError }) {
  ...
  return <div className="fr-capital-table-wrap"><table className="fr-room-matrix"><thead><tr><th>Artifact</th>{!artifactOnly && grants.slice(0, 4).map((grant) => <th key={grant.uid}>{text(grant.investor_name || grant.investor_email, 'Investor')}</th>)}<th>Visibility</th></tr></thead><tbody>{files.map((file, index) => <tr key={file.uid || index} data-testid={`row-data-room-file-${file.uid || index}`}><td><strong>{text(file.name, 'Untitled file')}</strong><small>{text(file.content_type, 'Type not recorded')}</small></td>{!artifactOnly && grants.slice(0, 4).map((grant) => { const state = fileAccess(file, grant); return <td key={grant.uid}><span className={`fr-room-pill is-${state.tone}`}>{state.label}</span></td>; })}<td><span className={`fr-room-pill ${file.visibility === 'nda' ? 'is-warn' : 'is-neutral'}`}>{file.visibility === 'nda' ? 'NDA required' : 'Open to invited'}</span></td></tr>)}</tbody></table>{!artifactOnly && grants.length > 4 && <p className="fr-room-overflow-note">{grants.length - 4} more active grants are available in the workspace.</p>}</div>;
}
function InvestorList({ grants }) { return grants.length ? <div className="fr-room-investors">{grants.map((grant) => <div key={grant.uid}><div><strong>{text(grant.investor_name || grant.investor_email, 'Investor')}</strong><span>{grant.nda_signed ? 'Pairwise NDA active' : 'NDA not recorded'}</span></div><span className={`fr-room-pill ${grantIsActive(grant) ? 'is-good' : 'is-neutral'}`}>{grantIsActive(grant) ? 'Active' : prettyAction(grant.status)}</span></div>)}</div> : <div className="fr-capital-inline-empty"><Users size={18} /><div><strong>No investor grants are recorded.</strong><p>Room access appears here only after the source returns a grant.</p></div></div>; }
...
function prettyAction(value) { return titleCase(value) || NOT_RECORDED; }
