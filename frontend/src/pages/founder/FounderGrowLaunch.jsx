...
export default function FounderGrowLaunch() {
  ...
  const visible = view === 'events' ? events : upcoming;
...
function LaunchContent({ project, events, attributions, upcoming, visible, view, setView, query, sourceErrors, leadsAttributed }) {
  ...
  return <div className="a5-sections"><div className="fg-launch-context"><div><span>Selected startup</span><strong data-testid="text-grow-launch-project">{text(project.name)}</strong></div><div><span>Launch source</span><strong>{calendarUnavailable ? 'Calendar unavailable' : events.length ? 'Project-linked calendar events' : 'No events recorded'}</strong></div></div>
        {/* Its four tabs are the zone header's now. Published and Articles are
        prose there: a calendar event carries no publication state, and no
        article record is connected to a startup at all. Both used to be live
        chips that reached an "unsupported" panel once clicked. */}
    <div className="fg-launch-tabs"><div className="fg-launch-actions"><Link to={`/calendar${query}`} data-testid="link-open-grow-launch-calendar"><CalendarDays size={13} /> Open calendar</Link></div></div>
    ...
    <section className="a5-card fg-launch-table"><Head title="Calendar" meta="Project-linked events · no inferred outcomes" />{calendarUnavailable ? <EmptyFeed error /> : <LaunchTable rows={visible} />}</section>
