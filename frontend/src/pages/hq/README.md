...
| `SecurityPage.jsx` | The HQ "Security" row (canvas Y2, decision A4) at `/admin/security`, with canvas H7 ("Governance") reconciled into it rather than drawn beside it: the privileged-action feed — Time · Actor · Tenant · Action · Target and reason, over a union of four stores under H7's five filters — sessions and impersonations with the platform-wide force re-auth, H7's Data access zone, KYC and deletion-request clocks; security events, AI safety, sanctions and DR named as not recorded, and the "Return to HQ view" overlay named as tenant-scoped and therefore U1. |
