  ...
  return (
    <div
      className={`relative rounded-2xl border p-6 flex flex-col bg-white ${
        tier.highlight ? 'border-violet-500 shadow-lg ring-1 ring-violet-500/30' : 'border-gray-200'
      }`}
      data-card
    >
      {tier.highlight && (
        <div className="absolute -top-3 left-6 px-3 py-0.5 rounded-full bg-violet-600 text-white text-xs font-medium flex items-center gap-1">
          <Sparkles size={11} aria-hidden="true" /> Most popular
        </div>
      )}
      <h3 className="text-xl font-bold text-gray-900">{tier.name}</h3>
      <p className="mt-1 text-sm text-gray-600 min-h-[40px]">{tier.tagline}</p>
      <div className="mt-5 flex items-baseline gap-1">
        <span className="text-4xl font-bold text-gray-900">${price}</span>
        <span className="text-sm text-gray-500">/{billing === 'annual' ? 'mo, billed annually' : 'month'}</span>
      </div>
      <Link
        to={tier.cta.to}
        className={`mt-5 inline-flex items-center justify-center min-h-[44px] px-4 rounded-lg text-sm font-medium transition-colors ${
          tier.highlight
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <header className="text-center max-w-2xl mx-auto mb-10">
          ...
          <p className="text-base text-gray-600">
            Start free. Upgrade when you raise. Annual billing saves about 20% on every paid plan.
          </p>
          <div role="radiogroup" aria-label="Billing period" className="mt-6 inline-flex rounded-full border border-gray-200 bg-white p-1">
            {['monthly', 'annual'].map((b) => (
              <button
                ...
                className={`min-h-[40px] px-5 rounded-full text-sm font-medium transition-colors ${
        ...
        <section aria-labelledby="founder-tiers" className="mb-16">
          <h2 id="founder-tiers" className="text-2xl font-bold text-gray-900 mb-1">For founders</h2>
          <p className="text-sm text-gray-600 mb-6">From your first cold validation to your Series A.</p>
          <div className="grid md:grid-cols-3 gap-6">
            {FOUNDER_TIERS.map((t) => <TierCard key={t.id} tier={t} audience="Founder" billing={billing} />)}
          </div>
        </section>

        <section aria-labelledby="investor-tiers" className="mb-16">
          <h2 id="investor-tiers" className="text-2xl font-bold text-gray-900 mb-1">For investors</h2>
          <p className="text-sm text-gray-600 mb-6">Angels, emerging managers, and institutional funds.</p>
          <div className="grid md:grid-cols-3 gap-6">
        ...
        <section aria-labelledby="faq" className="max-w-3xl mx-auto">
          <h2 id="faq" className="text-2xl font-bold text-gray-900 mb-6 text-center">Frequently asked</h2>
