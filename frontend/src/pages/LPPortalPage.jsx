...
export default function LPPortalPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-1">LP Investor Portal</h1>
      <p className="text-sm text-gray-600 mb-6">Portfolio performance, capital calls, and returns tracking</p>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard icon={PieChart} label="Portfolio Cos" value={portfolio?.total_projects || 0} color="text-violet-600" />
      </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            {investors.length === 0 ? (
              ...
              <div className="divide-y divide-gray-800/50">
                {investors.map(inv => (
                  <div key={inv.id} className="px-4 py-3 hover:bg-gray-50/30 cursor-pointer transition-colors"
                    ...
                    <div className="mt-2 w-full bg-gray-50 rounded-full h-1.5">
                    ...
                    {selectedLP.capital_calls.map(c => (
                      <div key={c.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg text-xs">
                        ...
                        <span className={`px-2 py-0.5 rounded-full ${
                          c.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-500/20 text-yellow-400'
                        }`}>{c.status}</span>
                      </div>
                    ))}
                  </div>
              ...
              {portfolio.projects.map(p => (
                <tr key={p.id} className="border-b border-gray-200/50 hover:bg-gray-50/30">
                  ...
                  <td className="px-4 py-2 text-center text-sm">
                    <span className={p.score >= 85 ? 'text-emerald-400' : p.score >= 70 ? 'text-blue-400' : 'text-gray-600'}>
                      {p.score || '—'}
                    </span>
                  </td>
