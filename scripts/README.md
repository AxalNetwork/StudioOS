...
| `check-magic-link-insert.mjs` | The cheap half of the same question: `/magic/start` answered under budget **and committed its token row**. Two verdicts, `start_latency` and `token_row_written`. That row is exactly where the outage died — D74 found `magic_link_tokens` newest at 2026-08-03 and the failing attempt wrote nothing, so the request never reached the INSERT that is the endpoint's first act. Needs **one** new secret (`MAGIC_PROBE_EMAIL`); `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` already exist, and it reads D1 through the Cloudflare query API — no `wrangler`, no install. **It cannot see delivery** and says so in every report: `waitUntil` commits the row and sends the mail separately. Exit 2 also covers "the token cannot read D1", which must never be reported as "no row was written". `.github/workflows/magic-link-insert-probe.yml`, guarded by `frontend/test/magic_link_insert_probe.test.mjs`; see `documentation/architecture/DECISIONS.md` D79. |
